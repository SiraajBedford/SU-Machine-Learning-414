...
class HMM(object):
    ...
    transition table and emission densities,

    or fit() to initialize these by fitting to provided data.

    Once the model has been fitted, one can use viterbi() for inferring hidden state sequences, forward() to compute the likelihood of signals, score() to
    ...
    def _ltrtrans(n_states):
        ...
        trans = np.zeros((n_states + 1, n_states + 1))#the plus ones are for  the non-emmiting states
        trans[-1, :] = 1. / n_states#normalisiation value
    ...
    def _viterbi(signal, trans, dists):
        ...
        n_states=trans.shape[0]-1#-1 since arrays start at 0
        T=signal.shape[1] #T is the number of observations
        lltable=np.zeros((n_states+1, T+1))
        seq=np.zeros(T, dtype='int')#to stor the best sequences
...
#        
#        emissonLLs=np.array([[s.logf(x.flatten()) for s in dists] for x in np.hsplit(signal, signal.shape[1])])
#        
#        for time in range(T):
...
#                lltable[state, time]+=emissonLLs[time, state]
#                backtable[state, time]=bestpred
#                
#        for state in range(n_states+1):
#            bestpred=np.argmax(np.log(trans[:,state])+lltable[:,T-1])# Last time step (t = n+1) - transition to final state
#            lltable[state, T]=np.log(trans[bestpred, state])+lltable[bestpred, T-1] 
...
#        t=0
#        
#        #get the log likelihood for t6he forst step
#        # (this is the transition probability)
#        loglik=np.log(trans[current_state][seq[0]])
#        current_state=seq[0]
#        
#        #loop through sequence and get the log sunm of the probabilities for all states in the sequence
#        
...
#            t+=1
#        
#        #do the last step (from state N-1 to N)
#        
#        transition_prob=np.log(trans[current_state][current_state+1])#<---last
#        state_prob=dists[current_state].loglik(signal[:, t])
#        loglik+= transition_prob+state_prob
#        
#        return loglik
...
#        N=len(dists)#length of distributions
...
#        return loglik
        ...
        for t in np.arange(1, T):
            for j in np.arange(N):
                ...
                for i in np.arange(N):
                    transition_prob = np.log(trans[i][j])
                    next_a[j] = np.logaddexp(next_a[j], transition_prob + current_a[i])#logsum exp trick
                next_a[j] += current_dist
            current_a = next_a
            next_a = np.zeros(N)
    ...
    def _calcstates(self, trans, dists):
        ...
        for s in self.signals:
            ...
            for v in seq:
                ...
                i += 1
            newtrans[seq[-1], -1] += 1
        newstates=np.where(used)[0] #unused states
        newtrans=newtrans[newstates, :]
        newtrans=newtrans[:, newstates]    
        rowsums=np.sum(newtrans, axis=1)# Normalize counts for transition probabilities for EACH element in row
        newtrans=newtrans/rowsums[:, np.newaxis]
        states=np.array(states[:, newstates[:-1]], dtype=bool)# Remove unused states by indexing
        
        return states, newstates, totalll
...
#            cov, means =self._updatecovs(oldstates)
#            dists = [Gaussian(mean=means[i], cov=covs[i]) for i in range(len(covs))]
...
#        return trans, dists, newLL, iters
    ...
    def sample(self):
        ...
        while state != fin_state:
            ...
            samples=np.vstack(
                    [samples, multivariate_normal.rvs(mean=dist.get_mean(), cov=dist.get_cov())]
                    )  
